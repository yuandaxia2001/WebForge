# -*- coding: utf-8 -*-
"""
Web Browser Use Runner - Executes the browser use agent.

This module provides a wrapper for running the browser use agent
using the local aiground framework.
"""
...
_PIPELINE_ROOT = Path(__file__).parent.parent.parent  # benchmark_agents_pipeline
...
class WebBrowserUseRunner:
    """
    Wrapper for running the browser use agent.

    This class provides a clean interface for running
    browser-based benchmark tasks using the local aiground framework.
    """
    ...
    def __init__(self, config: Dict[str, Any], save_trace: bool = True):
        ...
        Args:
            config: Configuration dict from pipeline config
            save_trace: Whether to save trace files (can be large)
        """
        self.config = config
    ...
    async def run(
        ...
        Args:
            ...
            output_dir: Directory to save outputs (default: ./output/task_solving)
            task_prompt: Original task prompt from tasks.jsonl
            ground_truth: Ground truth answer from tasks.jsonl
        ...
        try:
            # Import from local aiground - browser use agent
            from aiground.browser_agent_service.browser_use.web_browser_use_agent import (
                WebBrowserUseAgent,
            ...
            # Initialize tool property (reuse image_cfg defined above)
...
**Instructions:**
1. ALWAYS call `record_step` BEFORE any other tool to record your observation, reasoning, and planned action
2. Call `terminate` when the task is completed, providing the final answer or failure reason
"""

            # Execute
            max_steps = self.config.get('max_steps', 50)
            req = WebBrowserUseTaskRequest(
                task=full_task,
                max_steps=max_steps,
                start_url=start_url,
            ...
            LOGGER.info(f"Starting browser use task at: {start_url}")
            ...
            if result_path.exists():
                success = True
                LOGGER.info(f"Browser use task completed: {result_path}")
            else:
                LOGGER.warning(f"Browser use result not found: {result_path}")
            
            return success
        
        except asyncio.CancelledError:
            LOGGER.warning("Browser use task was cancelled")
            # Mark as cancelled so result.json won't be saved (allows retry)
            if cleanup:
                try:
                    await cleanup(cancelled=True)
                    cleanup = None  # Prevent double cleanup in finally
                except Exception as e:
                    LOGGER.warning(f"Cleanup during cancellation warning: {e}")
            raise  # Re-raise to signal cancellation
        except Exception as e:
            LOGGER.exception(f"Browser use error: {e}")
            return False
        finally:
            # Always cleanup browser resources, even if an exception occurred
            if cleanup:
                try:

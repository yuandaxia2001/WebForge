...
from aiground.framework.thirdparty.browser_use import Browser as BrowserUseBrowser
from aiground.framework.thirdparty.browser_use import BrowserConfig
        ...
        async with self.lock:
            try:
                ...
                if action == "go_to_url":
                    ...
                    LOGGER.info(f"Web search results: {search_response}")
                    ...
                    try:
                        ...
                        LOGGER.debug("Page load state timeout (5s) after web_search, continuing anyway")
                    except Exception as e:
                        LOGGER.debug(f"Page load state check failed after web_search: {e}, continuing anyway")
...
"""
                ...
                LOGGER.exception(f"Error executing browser action '{action}'")
    ...
    def __del__(self):
        """Note: Cleanup should be done explicitly, not in destructor.
        
        Do NOT use asyncio.run() here as it can interfere with the main event loop
        and close all httpx connections.
        """
        pass

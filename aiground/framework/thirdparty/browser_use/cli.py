...
from aiground.framework.thirdparty.browser_use import (
    Agent,
    Browser,
    BrowserConfig,
...
from aiground.framework.thirdparty.browser_use.agent.views import AgentSettings
from aiground.framework.thirdparty.browser_use.logging_config import addLoggingLevel
...
def update_config_with_click_args(
    config: dict[str, Any], ctx: click.Context
) -> dict[str, Any]:
...
def get_llm(config: dict[str, Any]):
    ...
    if os.getenv("OPENAI_API_KEY"):
        ...
        return langchain_anthropic.ChatAnthropic(
            model="claude-3.5-sonnet-exp", temperature=temperature
        )
...
class BrowserUseApp(App):
    ...
    def __init__(self, config: dict[str, Any], *args, **kwargs):
        ...
        self.config = config
        self.browser: Browser = None  # Will be set before app.run_async()
    ...
    def setup_richlog_logging(self) -> None:
        ...
        class BrowserUseFormatter(logging.Formatter):
            def format(self, record):
                if isinstance(record.name, str) and record.name.startswith(
                    "browser_use."
                ):
        ...
        try:
            if READLINE_AVAILABLE and self.task_history:
                for item in self.task_history:
                    readline.add_history(item)
                logger.debug(
                    f"Added {len(self.task_history)} items to readline history"
                )
            else:
    ...
    def on_input_key_up(self, event: events.Key) -> None:
        ...
        if self.history_index > 0:
            ...
            self.query_one("#task-input").cursor_position = len(
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        if event.input.id == "task-input":
            ...
            if task.strip() and (
                not self.task_history or task != self.task_history[-1]
            ):
    ...
    def update_browser_panel(self) -> None:
        ...
        if self.browser:
            ...
            try:
                ...
                if (
                    hasattr(self.browser, "_chrome_subprocess")
                    and self.browser._chrome_subprocess
                ):
                    try:
                ...
                # Then check if we have a playwright browser connection
                    ...
                    hasattr(self.browser, "playwright_browser")
            ...
            browser_info.write(
                f"[bold cyan]{browser_class}[/] Browser ({browser_status})"
            )
            browser_info.write(
            ...
            browser_info.write(
                f"CDP Port: {self.browser.config.chrome_remote_debugging_port}"
            )

            if window_width and window_height:
                browser_info.write(
                    f"Window: [blue]{window_width}[/] × [blue]{window_height}[/]"
                )
            ...
            if connected and hasattr(self, "agent") and self.agent:
                ...
                if (
                    ...
                    current_url = (
                        self.agent.browser_context.agent_current_page.url.replace(
                            "https://", ""
                        )
                        .replace("http://", "")
                        .replace("www.", "")[:36]
    ...
    def update_model_panel(self) -> None:
        ...
        if self.llm:
            ...
            if self.agent:
                ...
                model_info.write(
                    f"[white]LLM:[/] [blue]{self.llm.__class__.__name__} [yellow]{model_name}[/]"
                )
            ...
            if (
                ...
                and hasattr(self.agent.state, "history")
            ):
                ...
                if num_steps > 0:
                    ...
                    model_info.write(
                        f"[white]Avg tokens/step:[/] [green]{avg_tokens_per_step:,.1f}[/]"
                    )
                ...
                if num_steps > 0 and self.agent.state.history.history[-1].metadata:
                    ...
                    if step_tokens > 0:
                        tokens_per_second = (
                            step_tokens / step_duration if step_duration > 0 else 0
                        )
                        model_info.write(
                            f"[white]Avg tokens/sec:[/] [magenta]{tokens_per_second:.1f}[/]"
                        )
                ...
                if total_duration > 0:
                    model_info.write(
                        f"[white]Total Duration:[/] [magenta]{total_duration:.2f}s[/]"
                    )

                    # Calculate response time metrics
                    model_info.write(
                        f"[white]Last Step Duration:[/] [magenta]{step_duration:.2f}s[/]"
                    )
                ...
                if hasattr(self.agent, "running"):
                    if self.agent.running:
                        ...
                        and hasattr(self.agent.state, "paused")
                        and self.agent.state.paused
                    ):
    ...
    def update_tasks_panel(self) -> None:
        ...
        if self.agent:
                ...
                for msg in message_history:
                    if hasattr(msg, "message") and hasattr(msg.message, "content"):
                        ...
                        if (
                            ...
                            and "Your ultimate task is:" in content
                        ):
            ...
            current_step = (
                self.agent.state.n_steps if hasattr(self.agent, "state") else 0
            )
            ...
            if hasattr(self.agent, "state") and hasattr(self.agent.state, "history"):
                ...
                if history_items:
                    ...
                    for idx, item in enumerate(history_items, 1):
                        ...
                        if item.model_output and hasattr(
                            item.model_output, "current_state"
                        ):
                            ...
                            eval_prev = (
                                item.model_output.current_state.evaluation_previous_goal
                            )
                            if (
                                eval_prev and idx > 1
                                ...
                                eval_summary = (
                                    eval_summary.replace("Success", "✅ ")
                                    .replace("Failed", "❌ ")
                                    .strip()
                                )
                                tasks_info.write(
                                    f"   [tan]Evaluation:[/] {eval_summary}"
                                )
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="main-container"):
            ...
            with Container(id="info-panels"):
                ...
                with Container(id="top-panels"):
                    ...
                    with Container(id="browser-panel"):
                        yield RichLog(
                            id="browser-info", markup=True, highlight=True, wrap=True
                        )

                    # Model panel
                    with Container(id="model-panel"):
                        yield RichLog(
                            id="model-info", markup=True, highlight=True, wrap=True
                        )
            ...
            with Container(id="task-input-container"):
                yield Label(
                    "🔍 What would you like me to do on the web?", id="task-label"
                )
...
async def textual_interface(config: dict[str, Any]):
    ...
    try:
        ...
        context_config = BrowserContextConfig.model_validate(
            config.get("browser_context", {})
        )
    ...
    try:
        ...
        logger.info(
            f"Preparing {browser_type} browser ({headless_str}) with {model_name} LLM"
        )
...
@click.option(
    "--model",
    type=str,
    help="Model to use (e.g., gpt-4o, claude-3-opus-20240229, gemini-pro)",
)
@click.option("--debug", is_flag=True, help="Enable verbose startup logging")
@click.option(
    "--headless", is_flag=True, help="Run browser in headless mode", default=None
)
...
def main(ctx: click.Context, debug: bool = False, **kwargs):
    ...
    if debug:
        logger.debug(
            f"System info: Python {sys.version.split()[0]}, Platform: {sys.platform}"
        )
    ...
    try:
        ...
        logger.error(
            f"Error updating config with command line args: {str(e)}", exc_info=True
        )

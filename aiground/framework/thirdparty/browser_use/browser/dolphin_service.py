    ...
    async def authenticate(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(auth_url, json=auth_data) as response:
                if not response.ok:
                    raise Exception(
                        f"Failed to authenticate with Dolphin Anty: {await response.text()}"
                    )
    ...
    async def get_browser_profiles(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                f"{self.api_url}/browser_profiles", headers=headers
            ) as response:
                if not response.ok:
                    raise Exception(
                        f"Failed to get browser profiles: {await response.text()}"
                    )
                data = await response.json()
                return data.get(
                    "data", []
                )  # Return the profiles array from the response

    async def start_profile(
        self, profile_id: Optional[str] = None, headless: bool = False
    ) -> dict:
        """

...
from patchright.async_api import ElementHandle, FrameLocator, Page
from pydantic import BaseModel, ConfigDict, Field

from aiground.framework.thirdparty.browser_use.browser.views import (
    BrowserError,
    BrowserState,
    TabInfo,
    URLNotAllowedError,
)
from aiground.framework.thirdparty.browser_use.dom.clickable_element_processor.service import (
    ClickableElementProcessor,
)
from aiground.framework.thirdparty.browser_use.dom.service import DomService
from aiground.framework.thirdparty.browser_use.dom.views import (
    DOMElementNode,
    SelectorMap,
)
from aiground.framework.thirdparty.browser_use.utils import (
    time_execution_async,
    time_execution_sync,
)

if TYPE_CHECKING:
    from aiground.framework.thirdparty.browser_use.browser.browser import Browser
...
class BrowserContextConfig(BaseModel):
    ...
    no_viewport: bool = (
        True  # True is the default for headful mode - browser window size determines viewport
    )
...
class BrowserSession:
    def __init__(
        ...
        self.cached_state_clickable_elements_hashes: Optional[
...
class BrowserContext:
    def __init__(
        ...
        self.config = config or BrowserContextConfig(
            **(browser.config.model_dump() if browser.config else {})
        )
        ...
        self.agent_current_page: Optional[Page] = (
            None  # The tab the agent intends to interact with
        )
        self.human_current_page: Optional[Page] = (
            None  # The tab currently shown in the browser UI
        )
        ...
        try:
            ...
            if self._page_event_handler and self.session.context:
                try:
                    ...
                    self.session.context.remove_listener(
                        "page", self._page_event_handler
                    )
    ...
    def __del__(self):
        ...
        The cleanup should be done explicitly by calling await context.close().
        """
        pass
    ...
    async def _initialize_session(self):
        ...
        if not current_page:
            if (
                ...
                and not pages[0].url.startswith(
                    "chrome://"
                )  # skip chrome internal pages e.g. settings, history, etc
                and not pages[0].url.startswith(
                    "chrome-extension://"
                )  # skip hidden extension background pages
            ):
        ...
        for page in pages:
            if (
                not page.url.startswith("chrome-extension://")
                and not page.url.startswith("chrome://")
        ...
        try:
            ...
            async def on_visibility_change(data):
                ...
                if old_foreground.url != page.url:
                    old_foreground_url = (
                        truncate_url(old_foreground.url, 22)
                        if old_foreground
                        else "about:blank"
                    )
                    logger.warning(
                        f"👁️ Foregound tab changed by human from {old_foreground_url} "
                        f"to {truncate_url(page.url, 22)} ({source}) but agent will "
                        f"stay on {truncate_url(self.agent_current_page.url, 22)}"
                    )
            ...
            js_code = """() => {
                ...
                "onVisibilityChange_TABID", visibility_func_name
            )
    ...
    async def get_session(self) -> BrowserSession:
        ...
        if self.session is None:
            try:
                ...
                logger.error(
                    f"❌  Failed to create new browser session: {e} (did the browser process quit?)"
                )
                raise e
        return self.session
    ...
    async def get_agent_current_page(self) -> Page:
        ...
        logger.warning(
            "⚠️  Failed to get agent current page, falling back to first page"
        )
        if session.context.pages:
            page = session.context.pages[0]
            self.agent_current_page = page
    ...
    async def _create_context(self, browser: PlaywrightBrowser):
        ...
        if (
            ...
            and len(browser.contexts) > 0
            and not self.config.force_new_context
        ):
            ...
            self.browser.config.browser_binary_path
            and len(browser.contexts) > 0
            and not self.config.force_new_context
        ):
        ...
        required_permissions = [
            "clipboard-read",
            "clipboard-write",
        ]  # needed for google sheets automation
        if self.config.geolocation:
            required_permissions.append("geolocation")
        missing_permissions = [
            p for p in required_permissions if p not in self.config.permissions
        ]
    ...
    async def set_viewport_size(self, page: Page) -> None:
        ...
        try:
            ...
            if self.config.no_viewport is False:
                ...
                logger.debug(
                    f"Set viewport size to {self.config.window_width}x{self.config.window_height}"
                )
            # else:
    ...
    async def _wait_for_stable_network(self):
        ...
        try:
            ...
            while True:
                ...
                if (
                    ...
                    >= self.config.wait_for_network_idle_page_load_time
                ):
                    break
        ...
        logger.debug(
            f"⚖️  Network stabilized for {self.config.wait_for_network_idle_page_load_time} seconds"
        )
    ...
    def _is_url_allowed(self, url: str) -> bool:
        ...
        try:
            ...
            return any(
                ...
                or domain.endswith("." + allowed_domain.lower())
    ...
    async def navigate_to(self, url: str):
        ...
        try:
            ...
            logger.debug("Page load state timeout (5s) after navigate_to, continuing anyway")
        except Exception as e:
            logger.debug(f"Page load state check failed after navigate_to: {e}, continuing anyway")
    ...
    async def refresh_page(self):
        ...
        try:
            ...
            logger.debug("Page load state timeout (5s) after refresh_page, continuing anyway")
        except Exception as e:
            logger.debug(f"Page load state check failed after refresh_page: {e}, continuing anyway")
    ...
    async def get_state(self, cache_clickable_elements_hashes: bool) -> BrowserState:
        ...
        if cache_clickable_elements_hashes:
            ...
            if (
                ...
                and session.cached_state_clickable_elements_hashes.url
                == updated_state.url
            ):
    ...
    async def take_screenshot(self, full_page: bool = False) -> str:
        ...
        try:
            ...
            logger.debug("Page load state timeout (5s) before screenshot, continuing anyway")
        except Exception as e:
            logger.debug(f"Page load state check failed before screenshot: {e}, continuing anyway")
                ...
                if self.config.save_downloads_path:
                    try:
                        ...
                        unique_filename = await self._get_unique_filename(
                            self.config.save_downloads_path, suggested_filename
                        )
                        download_path = os.path.join(
                            self.config.save_downloads_path, unique_filename
                        )
                        ...
                        try:
                            ...
                            logger.debug("Page load state timeout (5s) after click, continuing anyway")
                        except Exception as e:
                            logger.debug(f"Page load state check failed after click: {e}, continuing anyway")
                        await self._check_and_handle_navigation(page)
                else:
    ...
    async def switch_to_tab(self, page_id: int) -> None:
        ...
        try:
            ...
            logger.debug("Page load state timeout (5s) after switch_to_tab, continuing anyway")
        except Exception as e:
            logger.debug(f"Page load state check failed after switch_to_tab: {e}, continuing anyway")
    ...
    async def _resize_window(self, context: PlaywrightBrowserContext) -> None:
        ...
        try:
                ...
                await cdp_session.send(
                    "Browser.setWindowBounds",
                    {
                        ...
                        "bounds": {
                            ...
                            + BROWSER_NAVBAR_HEIGHT,  # Add height for browser chrome
            ...
            logger.debug(
                f'Attempted to resize window to {window_size["width"]}x{window_size["height"]}'
            )
        except Exception as e:

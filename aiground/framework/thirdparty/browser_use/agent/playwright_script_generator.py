...
from aiground.framework.thirdparty.browser_use.browser.browser import BrowserConfig
...
class PlaywrightScriptGenerator:
    ...
    def _generate_context_options(self) -> str:
        ...
        # Format as keyword arguments string
        options_str = ", ".join(
            f"{key}={repr(value)}" for key, value in options_dict.items()
        )
        return options_str
    ...
    def _get_selector_for_action(
        self, history_item: dict, action_index_in_step: int
    ) -> Optional[str]:
        """
    ...
    def _map_go_to_url(self, params: dict, step_info_str: str, **kwargs) -> List[str]:
        ...
        if url and isinstance(url, str):
            ...
            script_lines.append(
                f"            # Skipping go_to_url ({step_info_str}): missing or invalid url"
            )
        return script_lines
        ...
        **kwargs,
        ...
        if selector and index is not None:
            clean_text_expression = (
                f"replace_sensitive_data({json.dumps(str(text))}, SENSITIVE_DATA)"
            )
    ...
    def _map_click_element(
        ...
        **kwargs,
        ...
        if action_type == "click_element_by_index":
            logger.warning(
                f"Mapping legacy 'click_element_by_index' to 'click_element' ({step_info_str})"
            )
        index = params.get("index")
        selector = self._get_selector_for_action(history_item, action_index_in_step)
        script_lines = []
    ...
    def _map_scroll_down(self, params: dict, step_info_str: str, **kwargs) -> List[str]:
        ...
        if amount and isinstance(amount, int):
            script_lines.append(
                f'            print(f"Scrolling down by {amount} pixels ({step_info_str})")'
            )
            script_lines.append(
                f"            await page.evaluate('window.scrollBy(0, {amount})')"
            )
        else:
            script_lines.append(
                f'            print(f"Scrolling down by one page height ({step_info_str})")'
            )
            script_lines.append(
    ...
    def _map_scroll_up(self, params: dict, step_info_str: str, **kwargs) -> List[str]:
        ...
        if amount and isinstance(amount, int):
            script_lines.append(
                f'            print(f"Scrolling up by {amount} pixels ({step_info_str})")'
            )
            script_lines.append(
                f"            await page.evaluate('window.scrollBy(0, -{amount})')"
            )
            ...
            script_lines.append(
                f'            print(f"Scrolling up by one page height ({step_info_str})")'
            )
            script_lines.append(
                "            await page.evaluate('window.scrollBy(0, -window.innerHeight)')"
            )
    ...
    def _map_send_keys(self, params: dict, step_info_str: str, **kwargs) -> List[str]:
        ...
        if keys and isinstance(keys, str):
            ...
            script_lines.append(
                f'            print(f"Sending keys: {keys} ({step_info_str})")'
            )
            script_lines.append(
                f"            await page.keyboard.press({escaped_keys})"
            )
            script_lines.append("            await page.wait_for_timeout(500)")
        else:
            script_lines.append(
                f"            # Skipping send_keys ({step_info_str}): missing or invalid keys"
            )
        return script_lines
    ...
    def _map_open_tab(self, params: dict, step_info_str: str, **kwargs) -> List[str]:
        ...
        if url and isinstance(url, str):
            escaped_url = json.dumps(url)
            script_lines.append(
                f'            print(f"Opening new tab and navigating to: {url} ({step_info_str})")'
            )
            ...
            script_lines.append(
                f"            # Skipping open_tab ({step_info_str}): missing or invalid url"
            )
        return script_lines
    ...
    def _map_close_tab(self, params: dict, step_info_str: str, **kwargs) -> List[str]:
        ...
        if page_id is not None:
            ...
            script_lines.append(
                f"            # Skipping close_tab ({step_info_str}): missing page_id"
            )
        return script_lines
    ...
    def _map_switch_tab(self, params: dict, step_info_str: str, **kwargs) -> List[str]:
        ...
        if page_id is not None:
            ...
            script_lines.append(
                f"            # Skipping switch_tab ({step_info_str}): missing page_id"
            )
        return script_lines

    def _map_search_google(
        self, params: dict, step_info_str: str, **kwargs
    ) -> List[str]:
        ...
        if query and isinstance(query, str):
            ...
            script_lines.append(
                f"            # Skipping search_google ({step_info_str}): missing or invalid query"
            )
        return script_lines
    ...
    def _map_drag_drop(self, params: dict, step_info_str: str, **kwargs) -> List[str]:
        ...
        script_lines = [
            f'            print(f"Attempting drag and drop ({step_info_str})")'
        ]
        if source_sel and target_sel:
            escaped_source = json.dumps(source_sel)
            escaped_target = json.dumps(target_sel)
            script_lines.append(
                f"            await page.drag_and_drop({escaped_source}, {escaped_target})"
            )
            script_lines.append(
                f"            print(f'  Dragged element {escaped_source} to {escaped_target}')"
            )
        elif all(c is not None for c in source_coords) and all(
            c is not None for c in target_coords
        ):
            sx, sy = source_coords
            tx, ty = target_coords
            script_lines.extend(
    ...
    def _map_extract_content(
        self, params: dict, step_info_str: str, **kwargs
    ) -> List[str]:
        goal = params.get("goal", "content")
        logger.warning(
            f"Action 'extract_content' ({step_info_str}) cannot be directly translated to Playwright script."
        )
        return [
            f"            # Action: extract_content (Goal: {goal}) - Skipped in Playwright script ({step_info_str})"
        ]
    ...
    def _map_action_to_playwright(
        ...
        if not isinstance(action_dict, dict) or not action_dict:
            return [
                f"            # Invalid action format: {action_dict} ({step_info_str})"
            ]
        ...
        if not action_type or params is None:
            if action_dict == {}:
                return [
                    f"            # Empty action dictionary found ({step_info_str})"
                ]
            return [
                f"            # Could not determine action type or params: {action_dict} ({step_info_str})"
            ]
        ...
        if handler:
            ...
            logger.warning(
                f"Unsupported action type encountered: {action_type} ({step_info_str})"
            )
            return [
                f"            # Unsupported action type: {action_type} ({step_info_str})"
            ]
    ...
    def generate_script_content(self) -> str:
        ...
        try:
            ...
            logger.error(
                f"Helper script not found at {helper_script_path}. Cannot generate script."
            )
        ...
        for step_index, item_dict in enumerate(self.history):
            ...
            if not isinstance(item_dict, dict):
                logger.warning(
                    f"Skipping step {step_index + 1}: Item is not a dictionary ({type(item_dict)})"
                )
                script_lines.append(
            ...
            if not isinstance(actions, list):
                script_lines.append(
                    f"            # Actions format is not a list: {type(actions)}"
                )
                previous_item_dict = item_dict
                continue
            ...
            for action_index_in_step, action_detail in enumerate(actions):
                ...
                step_info_str = (
                    f"Step {step_index + 1}, Action {action_index_in_step + 1}"
                )
                ...
                action_type = (
                    next(iter(action_detail.keys()), None)

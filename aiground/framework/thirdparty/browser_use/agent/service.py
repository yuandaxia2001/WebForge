...
from aiground.framework.thirdparty.browser_use.agent.gif import create_history_gif
from aiground.framework.thirdparty.browser_use.agent.memory.service import Memory
from aiground.framework.thirdparty.browser_use.agent.memory.views import MemoryConfig
from aiground.framework.thirdparty.browser_use.agent.message_manager.service import (
    MessageManager,
    MessageManagerSettings,
)
from aiground.framework.thirdparty.browser_use.agent.message_manager.utils import (
...
from aiground.framework.thirdparty.browser_use.browser.context import BrowserContext
from aiground.framework.thirdparty.browser_use.browser.views import (
    BrowserState,
    BrowserStateHistory,
)
from aiground.framework.thirdparty.browser_use.controller.registry.views import (
    ActionModel,
)
from aiground.framework.thirdparty.browser_use.controller.service import Controller
from aiground.framework.thirdparty.browser_use.dom.history_tree_processor.service import (
    DOMHistoryElement,
    HistoryTreeProcessor,
)
from aiground.framework.thirdparty.browser_use.exceptions import LLMException
from aiground.framework.thirdparty.browser_use.telemetry.service import ProductTelemetry
from aiground.framework.thirdparty.browser_use.telemetry.views import (
...
class Agent(Generic[Context]):
    ...
    def __init__(
        ...
        register_external_agent_status_raise_error_callback: Union[
        ...
        if (
            self.sensitive_data
            and not self.browser.config.new_context_config.allowed_domains
        ):
            LOGGER.error(
        ...
        self.browser.config.new_context_config.disable_security = (
            self.browser.config.disable_security
        )
    ...
    def _set_message_context(self) -> Optional[str]:
        if self.tool_calling_method == "raw":
            ...
            if self.settings.message_context:
                ...
                self.settings.message_context = (
                    f"Available actions: {self.unfiltered_actions}"
                )
        return self.settings.message_context

    def _set_browser_use_version_and_source(
        self, source_override: Optional[str] = None
    ) -> None:
    ...
    async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
        ...
        try:
            ...
            if self.tool_calling_method == "raw":
                ...
                context_lines = (
                    self._message_manager.settings.message_context or ""
                ).split("\n")
                non_action_lines = [
                    line
                    for line in context_lines
                    if not line.startswith("Available actions:")
                ]
            ...
            self.state.last_result = [
                ActionResult(
                    error="The agent was paused with Ctrl+C", include_in_memory=False
                )
            ]
    ...
    async def get_next_action(self, input_messages: List[BaseMessage]) -> AgentOutput:
        ...
        if self.tool_calling_method == "raw":
            ...
            try:
                ...
                parsed: Optional[AgentOutput] = response["parsed"]
        ...
        if response.get("parsing_error") and "raw" in response:
            ...
            if hasattr(raw_msg, "tool_calls") and raw_msg.tool_calls:
                ...
                parsed = self.AgentOutput(
                    current_state=current_state, action=[self.ActionModel(**action)]
                )
            else:
                parsed = None
        else:
        ...
        if not (
            hasattr(self.state, "paused") and (self.state.paused or self.state.stopped)
        ):
            log_response(parsed)
    ...
    async def run(
        ...
        from aiground.framework.thirdparty.browser_use.utils import SignalHandler
    ...
    async def multi_act(
        ...
        for i, action in enumerate(actions):
            if action.get_index() is not None and i != 0:
                ...
                orig_target_hash = (
                    orig_target.hash.branch_path_hash if orig_target else None
                )
                new_target = new_selector_map.get(action.get_index())  # type: ignore
                new_target_hash = (
                    new_target.hash.branch_path_hash if new_target else None
                )
    ...
    async def load_and_rerun(
        self, history_file: Union[str, Path, None] = None, **kwargs
    ) -> List[ActionResult]:
        """
    ...
    def save_history(self, file_path: Union[str, Path, None] = None) -> None:
    ...
    def _verify_llm_connection(self) -> bool:
        ...
        """
        LOGGER.debug(
            f"Verifying the {self.llm.__class__.__name__} LLM knows the capital of France..."
        )

        if (
            getattr(self.llm, "_verified_api_keys", None) is True
            or SKIP_LLM_API_KEY_VERIFICATION
        ):
    ...
    async def _run_planner(self) -> Optional[str]:
        ...
        if self.planner_model_name and (
            "deepseek-r1" in self.planner_model_name
            or "deepseek-reasoner" in self.planner_model_name
        ):

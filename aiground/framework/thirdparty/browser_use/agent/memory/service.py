...
from aiground.framework.thirdparty.browser_use.utils import time_execution_sync
...
class Memory:
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "mem0 is required when enable_memory=True. Please install it with `pip install mem0`."
            )
    ...
    def create_procedural_memory(self, current_step: int) -> None:
        ...
        memory_content = self._create(
            [m.message for m in messages_to_process], current_step
        )
        ...
        new_messages.append(
            ManagedMessage(message=memory_message, metadata=memory_metadata)
        )
        ...
        logger.info(
            f"Messages consolidated: {len(messages_to_process)} messages converted to procedural memory"
        )

...
from aiground.framework.thirdparty.browser_use.agent.views import ActionResult
...
def test_token_overflow_handling_with_real_flow(
    ...
    for i in range(200):  # Simulate 40 steps of interaction
        ...
        assert (
            message_manager.state.history.current_tokens
            <= message_manager.settings.max_input_tokens + 100
        )
        ...
        if i % 4 == 0:
            assert isinstance(
                message_manager.state.history.messages[-2].message, HumanMessage
            )

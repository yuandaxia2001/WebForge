...
if TYPE_CHECKING:
    from aiground.framework.thirdparty.browser_use.agent.views import AgentOutput
...
class MessageHistory(BaseModel):
    ...
    def add_message(
        ...
        if position is None:
            ...
            self.messages.insert(
                position, ManagedMessage(message=message, metadata=metadata)
            )
    ...
    def add_model_output(self, output: AgentOutput) -> None:
        ...
        self.add_message(
            msg, MessageMetadata(tokens=100)
        )  # Estimate tokens for tool calls

        # Empty tool response
        tool_message = ToolMessage(content="", tool_call_id="1")
        self.add_message(
            tool_message, MessageMetadata(tokens=10)
        )  # Estimate tokens for empty response
    ...
    def remove_last_state_message(self) -> None:
        ...
        if len(self.messages) > 2 and isinstance(
            self.messages[-1].message, HumanMessage
        ):

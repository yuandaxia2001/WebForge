...
class Registry(Generic[Context]):
        ...
        try:
            ...
            if sensitive_data:
                validated_params = self._replace_sensitive_data(
                    validated_params, sensitive_data
                )

            # Check if the action requires browser
            if "browser" in parameter_names and not browser:
                raise ValueError(
                    f"Action {action_name} requires browser but none provided."
                )
            if "page_extraction_llm" in parameter_names and not page_extraction_llm:
                raise ValueError(
                    f"Action {action_name} requires page_extraction_llm but none provided."
                )
            if "available_file_paths" in parameter_names and not available_file_paths:
                raise ValueError(
                    f"Action {action_name} requires available_file_paths but none provided."
                )

            if "context" in parameter_names and not context:
                raise ValueError(
                    f"Action {action_name} requires context but none provided."
                )
    ...
    def create_action_model(
        self, include_actions: Optional[List[str]] = None, page=None
    ) -> type[ActionModel]:
        ...
        return create_model(
            "ActionModel", __base__=ActionModel, **fields
        )  # type:ignore

...
from aiground.framework.thirdparty.browser_use.dom.views import DOMElementNode
...
class ClickableElementProcessor:
    ...
    def get_clickable_elements_hashes(dom_element: DOMElementNode) -> Set[str]:
        ...
        clickable_elements = ClickableElementProcessor.get_clickable_elements(
            dom_element
        )
        return {
            ClickableElementProcessor.hash_dom_element(element)
        ...
        for child in dom_element.children:
            if isinstance(child, DOMElementNode):
                ...
                clickable_elements.extend(
                    ClickableElementProcessor.get_clickable_elements(child)
                )
    ...
    def hash_dom_element(dom_element: DOMElementNode) -> str:
        parent_branch_path = ClickableElementProcessor._get_parent_branch_path(
            dom_element
        )
        branch_path_hash = ClickableElementProcessor._parent_branch_path_hash(
            parent_branch_path
        )
        attributes_hash = ClickableElementProcessor._attributes_hash(
        ...
        return ClickableElementProcessor._hash_string(
            f"{branch_path_hash}-{attributes_hash}-{xpath_hash}"
        )

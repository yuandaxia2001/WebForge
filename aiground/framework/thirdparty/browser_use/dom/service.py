...
class DomService:
    def __init__(self, page: "Page"):
        ...
        self.js_code = (
            resources.files("aiground.framework.thirdparty.browser_use.dom")
            .joinpath("buildDomTree.js")
            .read_text()
        )
    ...
    async def get_cross_origin_iframes(self) -> list[str]:
        ...
        hidden_frame_urls = (
            await self.page.locator("iframe")
            .filter(visible=False)
            .evaluate_all("e => e.map(e => e.src)")
        )

        is_ad_url = lambda url: any(
            domain in urlparse(url).netloc
            for domain in ("doubleclick.net", "adroll.com", "googletagmanager.com")
        )
        ...
        return [
            ...
            and urlparse(frame.url).netloc
            != urlparse(self.page.url).netloc  # exclude same-origin iframes

...
from aiground.framework.thirdparty.mcp.server import FastMCP
from aiground.framework.thirdparty.mcp.server import Server as LowLevelServer
        ...
        for name in ["mcp", "server", "app"]:
            if hasattr(module, name):
                if not _check_server_object(getattr(module, name), f"{file}:{name}"):
                    logger.error(
                        f"Ignoring object '{file}:{name}' as it's not a valid server object"
                    )
                    continue

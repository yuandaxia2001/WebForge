...
from aiground.framework.thirdparty.mcp.server.auth.middleware.auth_context import (
    AuthContextMiddleware,
)
from aiground.framework.thirdparty.mcp.server.auth.middleware.bearer_auth import (
    BearerAuthBackend,
    RequireAuthMiddleware,
)
from aiground.framework.thirdparty.mcp.server.auth.provider import (
    OAuthAuthorizationServerProvider,
    ProviderTokenVerifier,
    TokenVerifier,
)
from aiground.framework.thirdparty.mcp.server.auth.settings import AuthSettings
from aiground.framework.thirdparty.mcp.server.elicitation import (
    ElicitationResult,
    ElicitSchemaModelT,
    elicit_with_validation,
)
from aiground.framework.thirdparty.mcp.server.fastmcp.exceptions import ResourceError
from aiground.framework.thirdparty.mcp.server.fastmcp.prompts import (
    Prompt,
    PromptManager,
)
from aiground.framework.thirdparty.mcp.server.fastmcp.resources import (
    FunctionResource,
    Resource,
    ResourceManager,
)
from aiground.framework.thirdparty.mcp.server.fastmcp.tools import Tool, ToolManager
from aiground.framework.thirdparty.mcp.server.fastmcp.utilities.logging import (
    configure_logging,
    get_logger,
)
from aiground.framework.thirdparty.mcp.server.lowlevel.helper_types import (
    ReadResourceContents,
)
from aiground.framework.thirdparty.mcp.server.lowlevel.server import LifespanResultT
from aiground.framework.thirdparty.mcp.server.lowlevel.server import Server as MCPServer
from aiground.framework.thirdparty.mcp.server.lowlevel.server import (
    lifespan as default_lifespan,
)
from aiground.framework.thirdparty.mcp.server.session import (
    ServerSession,
    ServerSessionT,
)
...
from aiground.framework.thirdparty.mcp.server.sse import SseServerTransport
from aiground.framework.thirdparty.mcp.server.stdio import stdio_server
from aiground.framework.thirdparty.mcp.server.streamable_http import EventStore
from aiground.framework.thirdparty.mcp.server.streamable_http_manager import (
    StreamableHTTPSessionManager,
)
from aiground.framework.thirdparty.mcp.server.transport_security import (
    TransportSecuritySettings,
)
from aiground.framework.thirdparty.mcp.shared.context import (
    LifespanContextT,
    RequestContext,
    RequestT,
)
from aiground.framework.thirdparty.mcp.types import (
    AnyFunction,
    ContentBlock,
    GetPromptResult,
)
from aiground.framework.thirdparty.mcp.types import Prompt as MCPPrompt
from aiground.framework.thirdparty.mcp.types import PromptArgument as MCPPromptArgument
from aiground.framework.thirdparty.mcp.types import Resource as MCPResource
from aiground.framework.thirdparty.mcp.types import (
    ResourceTemplate as MCPResourceTemplate,
)
from aiground.framework.thirdparty.mcp.types import Tool as MCPTool
from aiground.framework.thirdparty.mcp.types import ToolAnnotations
...
def lifespan_wrapper(
    ...
    [MCPServer[LifespanResultT, Request]], AbstractAsyncContextManager[LifespanResultT]
]:
    @asynccontextmanager
    async def wrap(
...
class FastMCP(Generic[LifespanResultT]):
    def __init__(
        ...
        *,
        ...
        session_resource_manager: Optional[SessionResourceManager] = None,
    ):
        self._session_resource_manager = session_resource_manager
        ...
        if self.settings.auth is not None:
            ...
            if not auth_server_provider and not token_verifier:
                raise ValueError(
                    "Must specify either auth_server_provider or token_verifier when auth is enabled"
                )
        else:
    ...
    def streamable_http_app(self) -> Starlette:
        ...
        if self._session_manager is None:
            self._session_manager = StreamableHTTPSessionManager(
                ...
                security_settings=self.settings.transport_security,
                session_resource_manager=self._session_resource_manager,
            )

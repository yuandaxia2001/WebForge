...
from aiground.framework.thirdparty.mcp.server.auth.errors import (
...
from aiground.framework.thirdparty.mcp.shared.auth import OAuthToken
    ...
    async def handle(self, request: Request):
        ...
        if token_request.grant_type not in client_info.grant_types:
            return self.response(
                TokenErrorResponse(
                    ...
                    error_description=(
                        f"Unsupported grant type (supported grant types are {client_info.grant_types})"
                    ),
                )
            )
        ...
        match token_request:
            case AuthorizationCodeRequest():
                ...
                token_redirect_str = (
                    str(token_request.redirect_uri)
                    if token_request.redirect_uri is not None
                    else None
                )
                auth_redirect_str = (
                    str(authorize_request_redirect_uri)
                    if authorize_request_redirect_uri is not None
                    else None
                )
